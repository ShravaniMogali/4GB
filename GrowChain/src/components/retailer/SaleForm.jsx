  ...
  const handleSale = async (e) => {
    ...
    try {
      ...
      try {
        ...
        await updateConsignmentStatus(id, remainingQuantity > 0 ? CONSIGNMENT_STATUS.READY_FOR_SALE : CONSIGNMENT_STATUS.SOLD, consignment.currentLocation);
        ...
        if (blockchainError.message && blockchainError.message.includes('not found')) {
          try {
            ...
            toast.success('Consignment registered on blockchain and sale recorded');
          } catch (registerError) {
            console.error('Error registering consignment on blockchain:', registerError);
            toast.error('Failed to register on blockchain, but sale was recorded');
          }
        } else {
